  ...
  while len(all_reviews) < target_reviews:
    ...
    try:
      ...
      with urllib.request.urlopen(req) as response:
        ...
        for item in items:
          # Bắt chính xác trường ngày tháng đánh giá
          review_date = (
              item.get('CreatedOnTimeDiff') or item.get('CreatedDate') or ''
          )
          ...
          review_record = {
              ...
              'restaurant_rating': item.get(
                  'AvgRating', 0
              ),  # Điểm đánh giá chung nhà hàng
              'review_id': item.get('Id'),
              'reviewer_id': item.get('User', {}).get(
                  'Id'
              ),  # Mã định danh người review
              'review_text': item.get('Description', '').strip(),
              'rating': item.get('Rating', 0),
              'review_date': review_date,
              'crawl_timestamp': datetime.now().isoformat(),  # Mốc thời gian crawl
          }
